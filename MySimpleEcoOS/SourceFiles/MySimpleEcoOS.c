...
void PrintDuration(uint64_t duration) {
	...
	g_pIVFB->pVTbl->WriteString(g_pIVFB, 0, 0, 26, 1, CHARACTER_ATTRIBUTE_FORE_COLOR_LIGHT_RED, to_print, 25);
}
...
void PrintPercent(int32_t percent, uint32_t column, uint32_t row) {
	...
	digit = percent % 10;
	to_print[0] = '0' + digit;
	g_pIVFB->pVTbl->WriteString(g_pIVFB, 0, 0, column, row, CHARACTER_ATTRIBUTE_FORE_COLOR_LIGHT_RED, to_print, 3);
}
...
void printProgress() {
	...
	task_progress /= 5;
	g_pIVFB->pVTbl->WriteString(g_pIVFB, 0, 0, 0, 1, CHARACTER_ATTRIBUTE_FORE_COLOR_LIGHT_GREEN, "[", 1);
	...
	g_pIVFB->pVTbl->WriteString(g_pIVFB, 0, 0, 20, 1, CHARACTER_ATTRIBUTE_FORE_COLOR_LIGHT_GREEN, "] ", 2);
}
...
void Task1(uint64_t duration) {
	...
	task_progress = 0;
	++row_pointer;
    g_pIVFB->pVTbl->WriteString(g_pIVFB, 0, 0, 2, row_pointer, CHARACTER_ATTRIBUTE_FORE_COLOR_MAGENTA, "Task 1 has been started", 24);
	g_pIVFB->pVTbl->WriteString(g_pIVFB, 0, 0, 12, 0, CHARACTER_ATTRIBUTE_FORE_COLOR_MAGENTA, "Task 1", 6);
	...
	g_pIVFB->pVTbl->WriteString(g_pIVFB, 0, 0, 25, row_pointer, CHARACTER_ATTRIBUTE_FORE_COLOR_GREEN, " -> Task 1 has been finished", 28);
}
...
void Task2(uint64_t duration) {
	...
	uint64_t startTime = currentTime;
	++row_pointer;
	g_pIVFB->pVTbl->WriteString(g_pIVFB, 0, 0, 2, row_pointer, CHARACTER_ATTRIBUTE_FORE_COLOR_MAGENTA, "Task 2 has been started", 23);
    g_pIVFB->pVTbl->WriteString(g_pIVFB, 0, 0, 12, 0, CHARACTER_ATTRIBUTE_FORE_COLOR_MAGENTA, "Task 2", 6);
	task_progress = 0;
    while ( endTime >= currentTime) {
        if (changeTime <= currentTime) {
			task_progress = (changeTime - startTime) * 100 / duration;
	...
	g_pIVFB->pVTbl->WriteString(g_pIVFB, 0, 0, 25, row_pointer, CHARACTER_ATTRIBUTE_FORE_COLOR_GREEN, " -> Task 2 has been finished", 28);
}
...
void Task3(uint64_t duration) {
	...
	task_progress = 0;
	++row_pointer;
	g_pIVFB->pVTbl->WriteString(g_pIVFB, 0, 0, 2, row_pointer, CHARACTER_ATTRIBUTE_FORE_COLOR_MAGENTA, "Task 3 has been started", 23);
    g_pIVFB->pVTbl->WriteString(g_pIVFB, 0, 0, 12, 0, CHARACTER_ATTRIBUTE_FORE_COLOR_MAGENTA, "Task 3", 6);
    while ( endTime >= currentTime) {
        if (changeTime <= currentTime) {
			task_progress = (changeTime - startTime) * 100 / duration;
            printProgress();
	...
	g_pIVFB->pVTbl->WriteString(g_pIVFB, 0, 0, 25, row_pointer, CHARACTER_ATTRIBUTE_FORE_COLOR_GREEN, " -> Task 3 has been finished", 28);
}
...
void Task4(uint64_t duration) {
	...
	task_progress = 0;
	++row_pointer;
	g_pIVFB->pVTbl->WriteString(g_pIVFB, 0, 0, 2, row_pointer, CHARACTER_ATTRIBUTE_FORE_COLOR_MAGENTA, "Task 4 has been started", 23);
    g_pIVFB->pVTbl->WriteString(g_pIVFB, 0, 0, 12, 0, CHARACTER_ATTRIBUTE_FORE_COLOR_MAGENTA, "Task 4", 6);
    while ( endTime >= currentTime) {
        if (changeTime <= currentTime) {
			task_progress = (changeTime - startTime) * 100 / duration;
            printProgress();
	...
	g_pIVFB->pVTbl->WriteString(g_pIVFB, 0, 0, 25, row_pointer, CHARACTER_ATTRIBUTE_FORE_COLOR_GREEN, " -> Task 4 has been finished", 28);
}
...
void Task5(uint64_t duration) {
	...
	task_progress = 0;
	++row_pointer;
	g_pIVFB->pVTbl->WriteString(g_pIVFB, 0, 0, 2, row_pointer, CHARACTER_ATTRIBUTE_FORE_COLOR_MAGENTA, "Task 5 has been started", 23);
    g_pIVFB->pVTbl->WriteString(g_pIVFB, 0, 0, 12, 0, CHARACTER_ATTRIBUTE_FORE_COLOR_MAGENTA, "Task 5", 6);
    while ( endTime >= currentTime) {
        if (changeTime <= currentTime) {
			task_progress = (changeTime - startTime) * 100 / duration;
            printProgress();
	...
	g_pIVFB->pVTbl->WriteString(g_pIVFB, 0, 0, 25, row_pointer, CHARACTER_ATTRIBUTE_FORE_COLOR_GREEN, " -> Task 5 has been finished", 28);
}
...
*/
    ...
    result = pIScheduler->pVTbl->NewTask(pIScheduler, Task1, 3000000ul, 0x100, &pITask1);
	pIVFB->pVTbl->WriteString(pIVFB, 0, 0, 2, row_pointer, CHARACTER_ATTRIBUTE_FORE_COLOR_BLUE, "Task 1 has been received", 24);
	++row_pointer;
    result = pIScheduler->pVTbl->NewTask(pIScheduler, Task2, 5000000ul, 0x100, &pITask2);
	pIVFB->pVTbl->WriteString(pIVFB, 0, 0, 2, row_pointer, CHARACTER_ATTRIBUTE_FORE_COLOR_BLUE, "Task 2 has been received", 24);
	++row_pointer;
    result = pIScheduler->pVTbl->NewTask(pIScheduler, Task3, 2000000ul, 0x100, &pITask3);
	pIVFB->pVTbl->WriteString(pIVFB, 0, 0, 2, row_pointer, CHARACTER_ATTRIBUTE_FORE_COLOR_BLUE, "Task 3 has been received", 24);
	++row_pointer;
    result = pIScheduler->pVTbl->NewTask(pIScheduler, Task4, 8000000ul, 0x100, &pITask4);
	pIVFB->pVTbl->WriteString(pIVFB, 0, 0, 2, row_pointer, CHARACTER_ATTRIBUTE_FORE_COLOR_BLUE, "Task 4 has been received", 24);
	++row_pointer;
    result = pIScheduler->pVTbl->NewTask(pIScheduler, Task5, 1000000ul, 0x100, &pITask5);
	pIVFB->pVTbl->WriteString(pIVFB, 0, 0, 2, row_pointer, CHARACTER_ATTRIBUTE_FORE_COLOR_BLUE, "Task 5 has been received", 24);
	if (result != 0) {
        /* Освобождение в случае ошибки */
        goto Release;
    }
    ...
    pIVFB->pVTbl->WriteString(pIVFB, 0, 0, 1, 2, CHARACTER_ATTRIBUTE_FORE_COLOR_YELLOW, "Welcome to EcoOS!", 17);
